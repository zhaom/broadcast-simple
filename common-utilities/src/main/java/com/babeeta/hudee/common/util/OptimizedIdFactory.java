package com.babeeta.hudee.common.util;
...
/**
 * ID factory, optimize the performance of insert.
 * 
 */
public class OptimizedIdFactory {
	private final static Logger logger = LoggerFactory
	        .getLogger(OptimizedIdFactory.class);
	...
	static {
		try {
			// build a 2-byte machine piece based on NICs info
			{
				...
				logger.info("[OptimizedIdFactory] Machine Token = "
				        + machineToken);
			}
			...
			// otherwise
			{
				...
				logger.info("[OptimizedIdFactory] Proccess Token = "
				        + proccessToken);
			}
		} catch (java.io.IOException ioe) {
			logger.error("[OptimizedIdFactory] Exception!!!! {}",
			        ioe.getMessage());
	...
	/**
	 * @return get an optimize id
	 */
	public static String getOptimizedId() {
		byte b[] = new byte[16];
